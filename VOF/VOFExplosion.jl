...
function visualize_dam_break(domain::LBMDomain, frame::Int)
    ...
    heatmap(1:domain.nx, 1:domain.ny, transpose(fluid_mask), 
            ...
            title="timestep: $frame, explosion at 1000", yaxis = :flip)
end
...
function add_underwater_explosion!(domain::LBMDomain, explosion_x::Int, explosion_y::Int, radius::Int, force_magnitude::Float64)
    # Apply explosion force to nearby cells with proper radial direction
    for ix in max(1, explosion_x-radius):min(domain.nx, explosion_x+radius)
        for iy in max(1, explosion_y-radius):min(domain.ny, explosion_y+radius)
            # Check if cell is fluid or interface
            if (domain.cell_type[ix, iy] & (CellType.FLUID | CellType.INTERFACE)) != 0
                # Calculate distance from explosion center
                dist = sqrt((ix - explosion_x)^2 + (iy - explosion_y)^2)
                
                # Apply force if within explosion radius
                if dist <= radius
                    # Calculate direction vector from explosion center (radial)
                    dx = ix - explosion_x
                    dy = iy - explosion_y
                    ...
                    if dist > 0
                        ...
                        # Exponential force decay with distance
                        force_factor = force_magnitude * exp(-dist / radius)
                        
                        # Add radial velocity (force pointing outward in all directions)
                        domain.u[1, ix, iy] += force_factor * dx
                        domain.u[2, ix, iy] += force_factor * dy
                        
                        # The key fix: Update distribution functions according to the force direction
                        for n in 1:9
                            # Calculate dot product between lattice direction and force direction
                            # This ensures the force is properly distributed in the direction of explosion
                            dir_component = v[n][1] * dx + v[n][2] * dy
                            
                            # Apply directional force to distribution function
                            if dir_component > 0
                                domain.fin[n, ix, iy] += force_factor * t[n] * dir_component
                            end
                        end
                        
                        # Ensure mass conservation for interface cells
                        if (domain.cell_type[ix, iy] & CellType.INTERFACE) != 0
                            # Adjust mass to be consistent with the new distribution functions
                            domain.mass[ix, iy] = sum(domain.fin[:, ix, iy])
                        end
                    end
                end
            end
    ...
    # Update density, velocity and equilibrium after explosion
    for ix in 1:domain.nx
        for iy in 1:domain.ny
            if (domain.cell_type[ix, iy] & (CellType.FLUID | CellType.INTERFACE)) != 0
                ...
                # Extra step: Re-enforce proper bounds on velocities
                vel = sqrt(domain.u[1, ix, iy]^2 + domain.u[2, ix, iy]^2)
                maxvel = 0.2  # Setting a lower max velocity for stability
                if vel > maxvel
                    domain.u[:, ix, iy] .*= maxvel / vel
                end
            end
    ...
    # Run a special update_types! pass to properly handle cell conversions due to explosion
    update_types!(domain)
end

densityEvolution = zeros(170,170,1500)

function demo()
    println("Running FreeSurfaceLBM Dam Break Demo with Underwater Explosion")
    println("-------------------------------------------------------")
    
    # Run basic dam break simulation
    domain = LBMDomain(nx=170, ny=170)
    domain = init_dam_break(170, 170)
    
    # Evolve the simulation up to the explosion point
    evolve!(domain, 1000)
    
    # Add underwater explosion
    explosion_x = 85   # Middle of the domain x-wise
    explosion_y = 160   # Near the bottom
    explosion_radius = 10
    explosion_force = 50.0
    
    println("Triggering underwater explosion at ($explosion_x, $explosion_y)")
    add_underwater_explosion!(domain, explosion_x, explosion_y, explosion_radius, explosion_force)
    
    # Continue simulation after explosion
    evolve!(domain, 500)
    
    # Create final animation including explosion
    frames = 10:10:length(domain.cell_type_history)
    create_animation(domain, frames, "dam_break_with_explosion.gif")
    
    # Analyze results
    max_vel, coverage = analyze_dam_break(domain)
    
    # Create velocity field visualization for selected frames after explosion
    explosion_frames = [1000, 1100, 1200, 1300, 1400]
    for frame in explosion_frames
        if frame < length(domain.cell_type_history)
    ...
    println("  - dam_break_with_explosion.gif: Animation of dam break with underwater explosion")
